...
class CustomerTest {
	...
	void testAddLoanApplication() {
		...
		c.addLoanApplication(1000, 12, 1, 12000, 1000, 50, 5, 10, 20, 5, "Full Time", 100000, 2);
		...
		assertEquals(5, la.getInq_last_6mths());
		assertEquals(10, la.getNum_actv_bc_tl());
		assertEquals(20, la.getNum_actv_rev_tl());
		assertEquals("Full Time", la.getJobStatus());
		assertEquals(100000, la.getIncome());
		assertEquals(2, la.getLengthOfEmployment());
	}

	@Test
	void testGetLastLoanApplication() {
		Customer c = new Customer(0, "Jack", "Software Engineer");
		LoanApplication la = c.addLoanApplication(1000, 12, 1, 12000, 1000, 50, 600, 10, 20, 5 , "Full Time", 100000, 2);
